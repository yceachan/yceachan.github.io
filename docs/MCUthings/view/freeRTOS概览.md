...
> [!note]
>
> 低优先级任务持有锁，未释放已被中优先级抢占，中优先级任务时钟运行。
>
> ---> 高优先级任务定时唤醒，wait mutex，结果锁被占据（且永远没机会归还）-->taskH 阻塞等待，task M running ，task L ready但没有权力运行任务
>
> ----> 发生taskM > taskH 的优先级反转的事实。
>
> 解决方案：引入优先级继承，taskH wait mutex时，向os 查询owner，并临时提升他的优先级
