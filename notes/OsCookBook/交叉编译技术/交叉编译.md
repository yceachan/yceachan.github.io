...
| -Wall      | 打开warning                              |                        |
| -g         | 打开调试                                 |                        |
| -l<name>   | 链接name库,库文件的真正名字是`libname.a' | gcc main.o -lm  -o app |
| -L<path>   | 指定库文件搜索目录                       | gcc -L./local_libs     |
| -i<file>   | 等价于#include <file>                    |                        |
| -I <path>> | 指定头文件搜索目录                       | gcc -I./include        |
...
```
#一个高校的make模板
.PHONY: clean all   //将clean、all视为伪目标————无视目录下同名文件
app: *.o
	gcc -o app *.o 
src1.o: src1.c
	gcc -c src1.c //-c 默认生成同名.o 
src2.o: src2.c
	gcc -c src2.c //-c 默认生成同名.o
clean :          
	rm -f *.o app
